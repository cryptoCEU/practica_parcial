const getById = id => document.getElementById(id);
  ...
  const word_array = (text) =>
      clean_string(text)
      .replace(/[ 0-9,;:.]+/gm, " ") // dejar solo letras
      .split(" ")                       // dividr por espacios
      .filter((w) => (w!==""))         // remove empty string elem
  ...
  const word_index = () => {
    ...
    let ordered_sentences =
      word_array(text)
      .map(el => el.toLowerCase()) //para tener en cuenta las minusculas
      .sort();
    let result =
  ...
  // Buscar, ordenar y mostrar las palabras repetidas
  const search_words = () => {
    let {text, view, search} = getIO();
